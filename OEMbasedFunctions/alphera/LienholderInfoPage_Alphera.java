...
public class LienholderInfoPage_Alphera {
	public WebDriver driver;

	public LienholderInfoPage_Alphera(WebDriver driver) {
		this.driver = driver;
	...
	@FindBy(id = "saveandaddproduct") // To locate the button with id
										// 'saveandaddproduct' in DOM
	public WebElement saveandadd; // Declaring a WebElement named saveandadd

	public void saveAndAddProduct() // Function for selecting the
									// saveandaddproduct button
	{
		try {
			WebDriverWait wait = new WebDriverWait(driver, 20); // Setting
																// waiting time
																// of web driver
			wait.until(ExpectedConditions.elementToBeClickable(saveandadd));
			saveandadd.click(); // clicking the master save and add product
								// button
