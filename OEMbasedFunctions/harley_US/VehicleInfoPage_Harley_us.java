...
public class VehicleInfoPage_Harley_us {
	...
	public VehicleInfoPage_Harley_us(WebDriver driver)
	{
		this.driver=driver;
		...
		try{	
			if(!product.contains("PLANNED") && !product.contains("MAINTENANCE")){
				...
				if(product.contains("GAP"))
				{
					...
					financeAmount(financeamount);
					arp(apr);
				} else if(product.contains("GPS THEFT")){
					vehicleMSRP(vehiclemsrp);
					financeAmount(financeamount);			
					arp(apr);
	...
	public void entervehicleinfoHDUS(String product,String vehiclemsrp, String financeamount, String apr, String grossgapcost, String vehicleprice){
		vehicleMSRP = vehiclemsrp;
		financeAmount = financeamount;
		APR = apr;
		grossGapCost = grossgapcost;
