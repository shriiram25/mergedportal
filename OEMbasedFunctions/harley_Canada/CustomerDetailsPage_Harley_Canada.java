...
public class CustomerDetailsPage_Harley_Canada {
	...
	public CustomerDetailsPage_Harley_Canada(WebDriver driver) {
		this.driver = driver;
	...
	public void continueBtn(String product)  //Function to type the text in textbox with id cont-btn
	{
		try {			
			WebDriverWait wait = new WebDriverWait(driver, 20);  //Setting waiting time of web driver
			wait.until(ExpectedConditions.visibilityOf(Continue));   //Instructing web driver to wait until the visibility of element by its locator before it throws error
			...
			//writeExcel();
			if(product.contains("GAP")) //Checking if the product is equal to the value stored in product1
			{
