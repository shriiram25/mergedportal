...
public class ProductPage_PBL_us {

	public WebDriver driver;	
	
	public ProductPage_PBL_us(WebDriver driver) {
		this.driver = driver;
	...
	public void selectProduct(String product, String bundleSelection, String plan, String termMonth, String termMiles, String deductible, String firstServiceInterval, String finalServiceInterval, String registrationCode) { // , String dealerCost){
		try{
			...
			Select select = new Select(driver.findElement(By.id("product")));
	 		...
	 		selectProduct1(product, bundleSelection, plan, termMonth, termMiles, deductible, firstServiceInterval, finalServiceInterval, registrationCode);
		}
		catch(Exception e){
	...
	//<<<--Function for selecting a plan or a bundle.it check with the product and select the plan or Product.

	public void selectProduct1(String product, String bundleselection, String plan, String termMonth, String termMiles, String deductible, String firstServiceInterval, String finalServiceInterval, String registrationCode){
		...
		if(product.contains("MULTI-COVERAGE")){
			...
			if(bundleselection.toUpperCase().contains("PLATINUM")){	 			
				...
				wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//input[contains(@textvalue, 'Gold')]")));
				driver.findElement(By.xpath("//input[contains(@textvalue, 'Gold')]")).click();
			}
			else if(bundleselection.toUpperCase().contains("SILVER")){
				WebDriverWait wait = new WebDriverWait(driver, 30);
				wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//input[contains(@textvalue, 'Silver')]")));
				...
				if(bundleselection.toUpperCase().contains("P")){
					...
					wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//input[contains(@textvalue, '& Wheel')]")));
					//wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//input[@value = 'P')]")));
				...
				if(bundleselection.toUpperCase().contains("X")){
					WebDriverWait wait = new WebDriverWait(driver, 20);
					wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//input[contains(@textvalue, 'Windshield')]")));
					driver.findElement(By.xpath("//input[contains(@textvalue, 'Windshield')]")).click();	
				}
				if(bundleselection.toUpperCase().contains("D")){
					WebDriverWait wait = new WebDriverWait(driver, 20);
					wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//input[contains(@textvalue, 'Dent')]")));
					driver.findElement(By.xpath("//input[contains(@textvalue, 'Dent')]")).click();
				}			    	
				
				if(bundleselection.toUpperCase().contains("K")){
					WebDriverWait wait = new WebDriverWait(driver, 20);
					wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//input[contains(@textvalue, 'Key')]")));
		...
		selectTermMonth(termMonth);		
 		...
 		else if(product.contains("PORSCHE TERM PROTECTION") || product.contains("PORSCHE TERM PROTECTION PLUS")) {
				selectPlan(plan);
				selectTermMonthDropdown(termMonth);
				selectTermMilesDropdown(termMiles);
		}			
 		else if(product.contains("QUALITYSHIELD GAP PROTECTION") || product.contains("GAP PROTECTION") || product.contains("GAP PLUS")) {
				selectPlan(plan);
