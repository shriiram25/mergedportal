...
public class CustomerDetailsPage_PBL_us {

	public WebDriver driver;  //Creating object for WebDriver
	
	
	//Assigning Variables to the values to use those within the script
	public String product1 = "PFS MULTI-COVERAGE";   
	public String product2 = "QUALITYSHIELD GAP PROTECTION";
	public String product3 = "PFS GAP PROTECTION";
	public String product4 = "PFS VEHICLE SERVICE PROTECTION";
	public String product5 = "PFS LEASE-END PROTECTION";
	public String product6 = "PORSCHE TERM PROTECTION";
	public String product7 = "PORSCHE TERM PROTECTION PLUS";
	
	/* Constructor to initialize the object created for WebDriver */
	public CustomerDetailsPage_PBL_us(WebDriver driver) {
		this.driver = driver;
	...
	@FindBy(id = "custAgreementLang")
	public WebElement CustomerAgreementLanguage;
	
	public void selectCustomerAgreementLanguage(String customerAgreementLanguage){
		try{
			WebDriverWait wait = new WebDriverWait(driver, 20);  //Setting waiting time of web driver
			wait.until(ExpectedConditions.visibilityOf(CustomerAgreementLanguage));
			Select sel = new Select(CustomerAgreementLanguage);
			sel.selectByVisibleText(customerAgreementLanguage);
		}
		catch(Exception e){
			e.printStackTrace();
			GetScreenshot.captureScreenshot();  //Capturing screenshot
	...
	public void firstName(String firstName)  //Function to type the text in textbox with id first_name
	{
		try {
			...
			WebDriverWait wait = new WebDriverWait(driver, 20);  //Setting waiting time of web driver
			wait.until(ExpectedConditions.elementToBeClickable(fname));  //Instructing web driver to wait until the visibility of element by its locator before it throws error
			Thread.sleep(500L);
			fname.clear();
	...
	public void continueBtn(String product)  //Function to type the text in textbox with id cont-btn
	{
		try {			
			...
			//writeExcel();
			/*
			if(product.equalsIgnoreCase(product1)) //Checking if the product is equal to the value stored in product1
			{
			...
			else if(product.equalsIgnoreCase(product4)) //Checking if the product is equal to the value stored in product4
			{
			...
			*/
			...
			//writeExcel();  //Calling the function to write the data to excel
			Assert.assertTrue(e.getMessage(), false);
		}
	}
