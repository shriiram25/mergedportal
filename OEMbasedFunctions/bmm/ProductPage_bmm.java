...
public class ProductPage_bmm {

	public WebDriver driver;

	public ProductPage_bmm(WebDriver driver) {
		this.driver = driver;
	...
	public void selectProduct1(String product, String bundleselection, String plan, String termMonth, String termmiles,
			String deductible) throws InterruptedException {

		try {

			if (product.contains("VSC")) {
				selectPlan(plan);
				selectTermMonthDropdown(termMonth);
				if (!termmiles.equals("")) {
					selectTermMilesDropdown(termmiles);
				}
			} else if (product.contains("CPO")) {
				if (plan.equalsIgnoreCase("BMW CANADA BASE CPO 7 SERIES PROMOTION")) {
					clikPromotionalMeassgePopUP();
				}
				selectPlan(plan);
				selectTermMonthDropdown(termMonth);
			}
	...
	//<<<<<Updated the Locator for the Promotional pop up(BMBS).>>>>>//
	@FindBy(xpath = "//*[@id='new_cpo_popup']//*[@id='cpo_popup_ok']")
	public WebElement popupBMBS;

	public void clikPromotionalMeassgePopUP() {

		WebDriverWait wait = new WebDriverWait(driver, 30);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("overlay_form")));
		wait.until(ExpectedConditions.elementToBeClickable(popupBMBS));
		popupBMBS.click();
	...
	public void taxAmtPopupAlert() {
		try {
			WebDriverWait wait = new WebDriverWait(driver, 20);
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("overlay_form")));
			wait.until(ExpectedConditions.elementToBeClickable(taxAmtWarning));
			taxAmtWarning.click();
			Step_Definition.taxPopup = "Customer tax waring message popped-up.";
		} catch (Exception e) {
			GetScreenshot.captureScreenshot();
			ErrorMessage.captureErrorMessage("Customer tax waring message.");
			Assert.assertTrue(e.getMessage(), false);
		}
	}
